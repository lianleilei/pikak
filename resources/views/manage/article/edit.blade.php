								...
								<div class="tab-pane active" id="tab_0">
									<div class="portlet box green">
										...
										<div class="portlet-body form">
											<!-- BEGIN FORM-->
											<form action="/manage/article/store" class="form-horizontal" method="post" enctype="multipart/form-data">
											{{ csrf_field() }}
												<input type="hidden" name="type" value="1">
												<input type="hidden" name="new_id" value="{{$data->id}}">
												<div class="form-body">
													<div class="form-group">
														<label class="col-md-2 control-label">栏目</label>
														<div class="col-md-8">
															<select class="form-control input-circle" name="typeid">
																<option>请选择分类</option>
																@foreach($group as $k=>$vo)
																<option value="{{$vo->id}}" {{$data->typeid == $vo->id ? 'selected' : ''}}>{!! $nbsp[$k] !!}{{$vo->name}}</option>
																@endforeach
															</select>
														</div>
													</div>
													<div class="form-group">
														...
														<div class="col-md-8">
															<input type="text" class="form-control input-circle" placeholder="20字以内" name="title" required value="{{$data->title}}">
														</div>
													</div>
													<div class="form-group">
														<label class="col-md-2 control-label">关键词</label>
														<div class="col-md-8">
															<input type="text" class="form-control input-circle" placeholder="多个关键词之间用空格或者“,”隔开" name="keywords" value="{{$data->keywords}}">
														</div>
													</div>
													<div class="form-group">
														<label class="col-md-2 control-label">摘要</label>
														<div class="col-md-8">
															<textarea  rows="3" class="form-control input-circle" placeholder="" name="description">{{$data->description}}</textarea>
													...
													<div class="form-group">
														...
														<div class="col-md-8">
															<input type="file" class="form-control" name="thumb"> <span class="help-block m-b-none">请选择图片，如不做修改请略过 图片尺寸： 251*153</span>
														</div>
														<input type="hidden" name="thumb_db" value="{{$data->thumb}}">
													</div>
												</div>
