		...
		<div class="page-content-wrapper">
			<div class="page-content">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="btn-group">
							<a href="/manage/article/add" class="btn green">
					...
					<div class="col-md-6">
						...
						<form action="/manage/article" method="get">
							<label><input type="text" class="form-control input-small input-inline" placeholder="关键字搜索" aria-controls="sample_1" name="search"><button type="submit" class="btn green">搜索</button></label>
						</form>
						</div>
					</div>
					<div class="col-md-12">
						...
						<div class="portlet box green">
							...
							<div class="portlet-body">
								<div class="table-responsive">
									<table class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											<tr class="odd gradeX" id="tr_{{$vo->id}}">
												...
												<td>{{$vo->title}}</td>
												<td>{{$vo->click}}</td>
												<td>{{$data['tname'][$key]}}</td>
												<td>{{date("Y-m-d H:i:s",$vo->created_at)}}</td>
												@if($vo->status == 0)
													<td><a href="/manage/article/send/{{$vo->id}}" class="btn default btn-xs red">
														<i class="fa fa-share"></i> 发布内容 </a>
													</td>
												@else
												<td>已发布</td>
												@endif
				                                <td>
					                                <a href="/manage/article/edit/{{$vo->id}}" class="btn default btn-xs purple">
														<i class="fa fa-edit"></i> 修改 </a>
				                                	<a href="javascript:;" class="btn default btn-xs black" onclick="delete_news('{{$vo->id}}')">
									...
									{{ $data['list']->appends($data['where'])->links() }}
								</div>
							</div>
						</div>
